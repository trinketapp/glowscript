...
with instructions for how to use it. (This file is automatically updated when the repository is updated.) Store the file
...
the rapydscript-ng runtime library:
...
5) In the file "runtime.js", delete these lines:
...
*/
	...
	var VPython_import = null // can be visual or vis or vpython
	var VPython_names = []    // e.g. [box, cylinder] or [] if none specified (e.g. import vpython)
    var waitlist = [new RegExp('^def\\s*\\('), new RegExp('\\s*function[^\\(]\\('),
    ...
    function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=0; lineno<lines.length; lineno++) {
			...
			if (lang == 'vpython') {
		    	...
		    	// 'from visual import *'  // this is the default (or 'from vpython import *')
    ...
    var vp_other = ["display", "vec", "vector", "rate", "sleep", "update", "compound", "color", "extrusion", "paths", "shapes",
        	...
        	if (program.slice(0,7) == 'ERROR: ') {
	        	...
	        	if (options.lang == 'rapydscript') {
	        		...
	        		if ( VPython_import === null) {
		        		...
		        		prog += "  display=canvas\n  vector=vec\n"
	        		} else if (VPython_names.length > 0) { // e.g from visual or vis import box, cylinder
