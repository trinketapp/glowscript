...
*/
            ...
            if (line.search(patt) >= 0) {
                ...
                while (end >= 0) {
                    ...
                    switch (line[end]) {
	                    case ("'"):
	                    case ('"'):
	                        return [line.slice(0,end+1)+','+GS_wait+ending, true]
	                    case (')'):
	                        parencnt++
	                        break
	                    case ('('):
	                        parencnt--
	                        if (parencnt === 0) {
                            	if (name == 'wait' || name == '_') return [line.slice(0,end+1)+GS_wait+ending, true]
	                            if (name.length === 0) return [line.slice(0,end+1)+GS_wait+ending, true]
	                            return [line.slice(0,end+1)+name+','+GS_wait+ending, true]
	                        }
	                        break
	                    case (','):
	                        if (parencnt === 1) {
                                if (name == 'wait') return [line.slice(0,end+1)+GS_wait+ending, true]
    ...
    function preprocess(program, lang) { // manipulate the program source
    	...
    	var waitfcts = [] // list of functions containing Streamline elements such as rate
		...
		for (lineno=0; lineno<lines.length; lineno++) {
		    ...
		    indent = m[1]
		    line = m[2]
		    if (lang == 'javascript' && line.slice(0,2) == '//') continue
					...
					if (more) {
			    		...
			    		if (ret[1] && defindents.length > 0) { // need to ensure def or function has wait argument
			    			...
			    			if (!infunction) {
			    				...
			    				if (waitfcts.indexOf(definfo[2]) == -1)
			    						waitfcts.push(definfo[2]) // add to list of functions that contain Streamline items
				    			var end
			    				var start = definfo[0]
    	...
    	for (var i=0; i<waitfcts.length; i++) {
    		var fname = waitfcts[i]
    		var p
    		if (fname[0] == '.') p = new RegExp('(\\w*)\\.'+fname.slice(1)+'\\s*\\(', 'g')
    		else p = new RegExp('(\\w*)\\W*'+fname+'\\s*\\(', 'g')
    		while (true) {
    			var m = p.exec(newprogram)
    			if (m === null) break
    			if (m[1] == 'def' || m[1] == 'function') continue
    			var parencnt = 1
    			var n = p.lastIndex-1
    			var startarg = n+1
    			var lastcomma = 0
