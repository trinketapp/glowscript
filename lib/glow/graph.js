;(function () {
    ...
    property.declare( graph.prototype, {
	    ...
	    make_title: function (title) { // used with Flot
        	...
        	ctx.fillStyle = color.to_html_rgba(vec(1,1,1), 0) // zero opacity
				  ...
				  for (var i=0; i<data.length; i++) {
					  if (i === 0) {
						  if (this.__graph.fast) { // Flot
							  ...
							  if (this.__newdata.length === 0) {
								  if (this.__data.length > 0) xy = this.__data[this.__data.length-1]
								  else xy = data[0]
								  lastx = xy[0]
								  lasty = xy[1]
								  if (this.__data.length === 0) {
									  this.__newdata.push(xy)
									  continue
								  }
							  } else {
								  xy = this.__newdata[this.__newdata.length-1]
								  lastx = xy[0]
								  lasty = xy[1]
							  }
						  }
