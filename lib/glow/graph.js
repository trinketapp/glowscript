;(function () {
    ...
    property.declare( graph.prototype, {
	    ...
	    make_title: function (title) { // used with Flot
        	...
        	ctx.fillStyle = color.to_html(color.gray(0.95))
	    ...
	    __update: function() { // called once by plot function, if graph not yet activated
        	...
        	if (!this.fast) { // Plotly
		        ...
		        for (var i=0; i<this.graph_series.length; i++) {
	            	...
	            	else if (!s.__activated) { // graph has already been activated, but not this series
	                	s.__activated = true
	            		p = s.__realtype
	            		var col = color.to_html(s.__color)
	            ...
	            for (var j=0; j<indices.length; j++) {
	            	var k = indices[j]
	            	if (!this.graph_series[k].__visible) {
	            		xs[j] = []
	            		ys[j] = []
	            	}
	            }
	            ...
	            for (var i = 0; i < this.graph_series.length; i++) {
                    ...
                    if (s.__dot && s.__realtype == 'lines' && s.options.data.length > 0) {
            ...
            if (!activated) new render_graph(this)
	    } // end of update()
        ...
        this.remove = function() { // JavaScript forbids anything.delete(), so in preprocessing we change .delete to .remove
        	...
        	if (this.__realtype == 'lines') {
        		if (this.__dot) {
        			...
        			if (!this.__graph.fast) {
		        		...
		        		this.__dotobject.visible = false
        			}
        		}
        		if (this.__markers) {
        			this.__markers = false
        			if (!this.__graph.fast) {
        				...
        				this.__markerobject.visible = false
        			}
        		}
        	}
        	this.visible = false
        	this.__graph.__changed
        }
          ...
          this.plot = function (args) {
			  ...
			  if (this.__realtype != 'lines' || this.__markers) {
				  ...
				  //var pt, x, y, xscale, yscale, dx, dy, x1, x2, y1, y2, lastx, lasty, dt, wx, wy
				  var pt, x, y, dx, dy, lastx, lasty, dt, xy, wx, wy
				  var g = this.__graph
				  ...
				  if (this.__data.length === 0) xy = data[0]
				  else xy = this.__data[this.__data.length-1]
				  lastx = xy[0]
				  lasty = xy[1]
				  for (var i=0; i<data.length; i++) {
		        	  if (this.__interval > 0) {
					  ...
					  checkval(x)
					  checkval(y)
					  if (x < xmin_actual || xmin_actual === null) g.__xmin_actual = xmin_actual = x
					  if (x > xmax_actual || xmax_actual === null) g.__xmax_actual = xmax_actual = x
					  if (y < ymin_actual || ymin_actual === null) g.__ymin_actual = ymin_actual = y
					  if (y > ymax_actual || ymax_actual === null) g.__ymax_actual = ymax_actual = y
					  wx = xmax_actual - xmin_actual
					  wy = ymax_actual - ymin_actual
					  if (!(wx === 0 || wy === 0)) { // always plot the very first point
						  /*
						  xscale = g.__width/(x2-x1)
						  yscale = g.__height/(y2-y1)
						  dx = xscale*Math.abs(x-lastx)
						  dy = yscale*Math.abs(y-lasty)
						  */
						  dx = Math.abs(x-lastx)/wx
						  dy = Math.abs(y-lasty)/wy
						  if (dx < 0.01 && dy < 0.01 && dt < 100) continue // ignore if less than 1% change and dt < 100 ms
					  }
					  if (this.__graph.fast) this.__data.push(pt) // we have to give flot the entire data each time we render
		        	  else this.__newdata.push(pt) // with Plotly we can extend incrementally (though it's no faster)
				  }
				  lastx = x
				  lasty = y
				  if (!this.__graph.fast && this.__newdata.length > 0) {
					  var d = this.__newdata[this.__newdata.length-1]
		        	  if (this.__dot ) this.__dotobject.data = [ d ]
				  }
			  } // end of gcurve
          ...
          if (this.__data.length > 0) this.plot(this.__data) // will start the rendering
    ...
    property.declare( gobject.prototype, {
    	...
    	data: {
	        ...
	        set: function (value) {
                this.__data = []
                if (this.__graph.fast) this.plot(value)
                else {
                    if (this.__activated) {
    	                var xs = []
    	                var ys = []
    	                ...
    	                Plotly.restyle(this.__graph.__id, {x:[xs], y:[ys]}, this.__id)
                    } else {
                    	this.plot(value)
                    }
                }
        	...
        	set: function (value) {
                ...
                if (this.__activated) {
                	if (value) {
			        	...
			        	else Plotly.restyle(this.__graph.__id, up, this.__id)
		        	} else {
		        		var up = {}
		        		up['marker.size'] = 0.1
			        	if (this.__graph.fast) this.options.points = {show: false, radius:r}
			        	else Plotly.restyle(this.__graph.__id, up, this.__markerobject.__id)
			        	this.__graph.__changed = true
		        	}
	        	}
        	}
	        ...
	        set: function (value) {
                ...
                if (this.__activated) {
                	if (this.__graph.fast) {
			        	...
			        	Plotly.restyle(this.__graph.__id, up, this.__id)
                	}
                }
            }
	    },
    	marker_color: {
	        ...
	        set: function (value) {
                ...
                var col = color.to_html(value)
                if (this.__graph.fast) this.options.__marker_color = col
                if (this.__activated) {
                	if (this.__graph.fast) {
    	        		...
    	        		if (this.__markerobject.__activated) {
    		                var up = {}
    		                up['marker.color'] = col
                            Plotly.restyle(this.__graph.__id, up, this.__markerobject.__id)
            			}
                	}
                }
            }
	    ...
	    label: {
	        ...
	        set: function (value) {
                ...
                this.__label = value
                if (this.__graph.fast) this.options.__label = value
                if (value.length > 0) this.__legend = true
                if (this.__activated) {
                	if (this.__graph.fast) {
    	                this.options.label = value
    	                this.__graph.__changed = true
                	} else Plotly.restyle(this.__graph.__id, {'name':value, 'showlegend':this.__legend}, this.__id)
	            }
	        }
	    },
	    legend: {
	        ...
	        set: function (value) {
                ...
                this.__legend = value
                if (this.__graph.fast) this.options.__legend = value
                if (this.__activated) {
                	if (this.__graph.fast) {
                		...
                		this.__graph.__changed = true
                	} else Plotly.restyle(this.__graph.__id, {'showlegend':value}, this.__id)
                }
        	}
	    },
	    delta: {
	        ...
	        set: function (value) {
                ...
                if (this.__activated) {
                	if (this.__graph.fast) {
    	                this.__graph.__changed = true
                	} else Plotly.restyle(this.__graph.__id, {'width':value}, this.__id)
                }
        	}
	    },
	    width: {
	        ...
	        set: function (value) {
                ...
                if (this.__activated) {
                	if (this.__graph.fast) {
		                ...
		                if (value) Plotly.restyle(this.__graph.__id, {'orientation':'h'}, this.__id)
		                else Plotly.restyle(this.__graph.__id, {'orientation':'v'}, this.__id)
                	}
                }
            }
	    },
	    orientation: {
	        ...
	        set: function (value) {
                ...
                if (this.__activated) {
                	...
                	else Plotly.restyle(this.__graph.__id, {'orientation':value}, this.__id)
                }
	        }
		},
	    dot: {
		...
		dot_color: {
	        ...
	        set: function (value) {
                ...
                var col = color.to_html(value)
            	if (this.__graph.fast) {
                    this.options.__dot_color = col
                    this.__graph.__changed = true
            	} else if (this.__dotobject.__activated) {
	                var up = {}
	                up['marker.color'] = col
                    Plotly.restyle(this.__graph.__id, up, this.__dotobject.__id)
    			}
            }
		},
		dot_radius: {
	        ...
	        set: function (value) {
                ...
                if (this.__graph.fast) {
                	this.options.__dot_radius = value
                    this.__graph.__changed = true
                } else if (this.__dotobject.__activated) {
		                var up = {}
	                    up['marker.size'] = 2*value
	        ...
	        set: function (value) {
                ...
                if (this.__activated) {
                	if (this.__graph.fast) {
                		this.__visible = value
    	                this.__graph.__changed = true
                	} else Plotly.restyle(this.__graph.__id, {visible:value}, this.__id)
                }
            }
		}
